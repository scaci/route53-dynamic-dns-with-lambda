...
define Package/ddns-scripts_aws-ddns/description
                Dynamic DNS Client scripts extension for Amazon AWS DDNS.
                                https://github.com/awslabs/route53-dynamic-dns-with-lambda
                It requires:
                "option username" to be a valid AWS access key id
                "option password" to be the matching AWS secret key id
                "option domain" to contain the hosted zone ID
                                "option paramopt" to define application api
endef
...
define Package/ddns-scripts_aws-ddns/preinst
                ...
                exit 0  # suppress errors
endef
define Package/ddns-scripts_aws-ddns/install
                $(INSTALL_DIR) $(1)/etc/uci-defaults
                $(INSTALL_BIN) $(PKG_BUILD_DIR)/files/ddns.defaults $(1)/etc/uci-defaults/ddns_aws-ddns
                $(INSTALL_DIR) $(1)/usr/lib/ddns
                $(INSTALL_BIN) $(PKG_BUILD_DIR)/files/update_aws_ddns.sh $(1)/usr/lib/ddns
                $(INSTALL_DIR) $(1)/usr/lib/ddns
                $(INSTALL_BIN) $(PKG_BUILD_DIR)/files/update_aws_ddns_ipv6.sh $(1)/usr/lib/ddns
                $(INSTALL_DIR) $(1)/usr/sbin
                $(INSTALL_BIN) $(PKG_BUILD_DIR)/files/aws-ddns $(1)/usr/sbin
endef
define Package/ddns-scripts_aws-ddns/postinst
                #!/bin/sh
                # remove old services file entries
                /bin/sed -i '/aws-ddns/d' $${IPKG_INSTROOT}/etc/ddns/services            >/dev/null 2>&1
                /bin/sed -i '/aws-ddns/d' $${IPKG_INSTROOT}/etc/ddns/services_ipv6      >/dev/null 2>&1
                # and create new
                printf "%s\\t%s\\n" '"aws-ddns"' '"update_aws_ddns.sh"' >> $${IPKG_INSTROOT}/etc/ddns/services
                printf "%s\\t%s\\n" '"aws-ddns"' '"update_aws_ddns_ipv6.sh"' >> $${IPKG_INSTROOT}/etc/ddns/services_ipv6
                # on real system restart service if enabled
                [ -z "$${IPKG_INSTROOT}" ] && {
                                [ -x /etc/uci-defaults/ddns_aws-ddns ] && \
                                                /etc/uci-defaults/ddns_aws-ddns && \
                                                                rm -f /etc/uci-defaults/ddns_aws-ddns >/dev/null 2>&1
                                /etc/init.d/ddns enabled \
                                                && /etc/init.d/ddns start >/dev/null 2>&1
                }
                exit 0  # suppress errors
endef
define Package/ddns-scripts_aws-ddns/prerm
                #!/bin/sh
                # if NOT run buildroot then stop service
                [ -z "$${IPKG_INSTROOT}" ] && /etc/init.d/ddns stop                                                   >/dev/null 2>&1
                # remove services file entries
...
$(eval $(call BuildPackage,ddns-scripts_aws-ddns))
